"""
Terminal Skill — Trinity's Runtime Sandbox
...
def test_skill(skill_name: str, fn_name: str, args_json: str = "[]", timeout: int = 15) -> str:
    ...
    Example — verify notes.add_note works:
        <skill:terminal.test_skill>notes|add_note|["Test note from executor"]</skill:terminal.test_skill>

    Integration workflow:
        1. self_improvement.audit("my_skill")       → finds issue
        2. self_improvement.fix("my_skill", ...)    → applies patch
        3. terminal.test_skill("my_skill", "run", '["arg1"]')  → confirms fix
    """
    timeout = _clamp_timeout(timeout)
...
def run_bash(command: str, timeout: int = DEFAULT_TIMEOUT) -> str:
    ...
    No shell chaining characters, and base command must be in the whitelist.
...
def status() -> str:
    """Show terminal capabilities, limits, and integration guide."""
    return "\n".join([
        "⚡ Terminal — Trinity's Runtime Sandbox",
        "=" * 52,
        "",
        "Functions:",
        ...
        "  3. terminal.test_skill('my_skill', 'fn', '[\"arg\"]')  → verify it works",
...
def calc(expression: str) -> str:
    ...
    Example:
        <skill:terminal.calc>sqrt(144) + 2**8</skill:terminal.calc>
    """
    try:
