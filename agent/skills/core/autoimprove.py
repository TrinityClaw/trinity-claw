...
Available loops:
  ...
  • ast_audit     — auto-fix deterministic AUTO_FIXABLE issues in dynamic skills
    ...
    if source == "discover_patterns":
        ...
        if match:
            ...
            if error_type not in patterns:
                ...
                all_ideas = _load_ideas()
                for idx, item in enumerate(all_ideas):
                    if item.get("id") == iid:
                        all_ideas[idx] = idea
                        break
                _save_ideas(all_ideas)
...
def run_all(max_experiments=5, max_runtime_seconds=25) -> str:
    ...
    for loop_name in ("daily_review", "ast_audit", "error_reduce", "suggest_core", "pattern_mining"):
        ...
        if "ERROR" in loop_result or "Error" in loop_result or "raised " in loop_result or "âŒ" in loop_result or "❌" in loop_result:
